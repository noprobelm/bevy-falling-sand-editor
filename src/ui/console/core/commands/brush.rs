...
impl Plugin for BrushCommandPlugin {
    fn build(&self, app: &mut App) {
        app.add_observer(on_update_brush_type)
            ...
            .add_observer(on_update_brush_spawn_mode)
...
impl ConsoleCommand for BrushSetModeSpawnDynamicRigidBodiesCommand {
    ...
    fn execute_action(
        ...
        console_writer.write(PrintConsoleLine::new(
            "Setting brush mode to 'avian'".to_string(),
        ));
...
fn on_update_brush_spawn_mode(
    trigger: On<BrushSetSpawnModeEvent>,
    mut brush_mode_state_next: ResMut<NextState<BrushMode>>,
...
fn on_show_brush_info(
    _trigger: On<ShowBrushInfoEvent>,
    ...
    mut console_writer: MessageWriter<PrintConsoleLine>,
) {
...
fn on_set_brush_particle(
    trigger: On<BrushSetParticleEvent>,
