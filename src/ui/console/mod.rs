...
impl Console {
    pub fn render(
        ...
        command_writer: &mut MessageWriter<ConsoleCommandEntered>,
            ...
            if console_state.expanded {
                ui.vertical(|ui| {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        ui.input_mut(|i| {
                            i.events.retain(
                            |message| !matches!(message, egui::Event::Text(text) if text.contains('`')),
                        );
                        });
...
pub fn receive_console_line(
    ...
    for message in messages.read() {
        console_state.add_message(message.line.clone());
    }
}
